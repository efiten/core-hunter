...
- **Payoff accepted:** terrain earns its keep in the Ardennes and the Eifel and shows little where most hunting happens. It ships anyway, with an opt-out.
...
1. **A terrain button on the rail**, the sixth FAB, on or off with the accent ring, on by default. Kasper's call over the select-with-off the issue proposed: turning terrain on while driving through a sheet is three taps. Settings keeps the exaggeration.
