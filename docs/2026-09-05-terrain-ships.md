...
- **Payoff accepted:** terrain earns its keep in the Ardennes and the Eifel and shows little where most hunting happens. It ships anyway, with the 3D view.
...
1. **The 3D view is the terrain switch.** No button of its own: 3D takes the terrain and the exaggeration along at once (Kasper, 2026-09-06, the same rule the map follows in #595). The first cut had a sixth FAB, on by default, then off by default, both the same morning; a switch beside the view was one more thing to explain for a state the view already names. Settings keeps the exaggeration.
...
3. **Hillshade comes with 3D; the mesh waits for the tiles.** The mesh, `setTerrain`, is what froze weak GPUs in #247 and makes `easeTo({pitch})` a no-op, so it needs the 3D view and DEM tiles for the view. Until the tiles are in, the map is flat, whatever the view says: that is the answer to what #394 left open, a slow host degrades to flat, never to a stalled map. Shading tracks the geometry (`hillshade-exaggeration` = exaggeration / 10). In 2D nothing is drawn: there is nothing to raise.
4. **Pitch and mesh in the right order.** Leaving 3D drops the mesh before the tilt back; entering 3D tilts first and sets the mesh once the camera has settled. Both follow from the plan, so a view tap mid-tilt lands in the same state.
5. **A style swap re-mounts terrain** like the sky: `setStyle` drops the source, `addOverlays` puts it back under the signal layers.

## Measured

Browser at 780 px, the DEM reachable (first cut, with the button): 2D mounts with the hillshade visible and no mesh; the 3D tap eases to pitch 60 and the mesh appears at 7 once the tiles are in; back to 2D the mesh goes before the tilt returns to 0. `idle` keeps firing with terrain in, which is the event a theme switch waits on. After the switch moved to the view (2026-09-06): 2D flat with no shading, 3D shading and mesh, the exaggeration change re-deriving the mesh.
