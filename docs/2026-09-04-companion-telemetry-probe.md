...
Kasper: "geen flood iig". The firmware floods a request when the contact's `out_path_len` is unknown (0xFF), and source-routes over a stored path otherwise, which may be stale. So the app runs the contact-path dance from coredrive-rx (`app/src/contactpath.js`): read the contact (`CMD_GET_CONTACT_BY_KEY`), force `out_path_len` to 0 for the ask (`CMD_ADD_UPDATE_CONTACT`, byte 35), send the request, put the contact back exactly as it was. An override that does not ack means no ask this cycle, and so does a contact read that gets no answer: a contact added from its advert starts with the unknown route, so asking without the reading can flood. A session that dies between the override and the restore leaves a record in localStorage, replayed on the next connect to the same companion. There is one record per companion and contact, each cleared only when its own restore acks, so a later ask to another contact, or from another companion, cannot overwrite a restore still owed. Where storage refuses that record (Safari with cookies blocked, a locked-down webview), there is no override and no ask: an override with no record has no way back. The replay and an ask take turns on the link: neither reply carries a correlator, and an ask that read a contact the replay was about to put back would go out over the restored route.
