...
- **Popups** are MapLibre popups, one at a time; their open/close reaches the name-resolution deferral (#271) as before. The hex cell's tooltip is a closeless popup that follows the pointer, since MapLibre has none. A tap on a touch screen shows it too: the browser follows a tap with a compatibility `mousemove`, which is what the line listens to.
