...
1. **Every registry of the companion's spreading factor is asked, at once** (`resolversFor`). A registry of another SF names nodes this radio cannot hear, so it is left out; with the SF unknown, all are asked. The order in `config.json` no longer decides the name, only where its position comes from (point 3).
