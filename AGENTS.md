...
**`changelog.json`**, a hand-written list of changes a user could actually notice.
...
- Two copies, `app/changelog.json` and `web/changelog.json`, kept byte-identical — neither
  deploy path can read a file outside its own directory. `web/parity.test.js` pins them.
