...
// for (OpenFreeMap serves Noto Sans). `glyphs` is whether the style has a
// glyphs URL at all: MapLibre refuses a text-field without one, so the bare
// fallback style (no host, no network) gets the ▲ and no names, rather than
// no layer.
export function nodeGlyphLayers({ bg, surface, font = ['Noto Sans Regular'], glyphs = true }) {
  const names = glyphs ? {
    layout: { 'text-field': ['get', 'label'], 'text-font': font, 'text-size': ['case', ['get', 'sel'], 12, 11],
      'text-anchor': 'left', 'text-offset': [1, 0], 'text-allow-overlap': true, 'text-ignore-placement': true, 'text-max-width': 40 },
    paint: { 'text-color': ['get', 'color'], 'text-halo-color': ['case', ['get', 'sel'], surface, bg],
      'text-halo-width': ['case', ['get', 'sel'], 2.5, 1.2], 'text-halo-blur': ['case', ['get', 'sel'], 0, 0.5],
      'text-opacity': ['get', 'op'] },
  } : { layout: {}, paint: {} }
  return [
    { id: NODE_DOT_LAYER, type: 'circle', source: NODE_DOT_SOURCE, layout: {},
    ...
    { id: NODE_ADVERT_LAYER, type: 'symbol', source: NODE_GLYPH_SOURCE,
      layout: { 'icon-image': TRI_IMAGE, 'icon-size': 1, 'icon-allow-overlap': true, 'icon-ignore-placement': true, ...names.layout },
      paint: { 'icon-color': ['get', 'color'], 'icon-halo-color': bg, 'icon-halo-width': 1, 'icon-opacity': ['get', 'op'], ...names.paint } },
  ]
}
