...
import { octagonRing, pillarRadiusM, createPillarCollapser } from './pointmarker.js'
...
export function createHuntMap(containerId) {
  ...
  // One collapser per map, so the answer survives across ticks (#462): the
  // collapse depends on the records alone, and with a static store the same
  // rows were being collapsed sixty times a minute for an identical result.
  // Everything that does change per tick -- ring geometry (zoom), tier colour
  // and age fade -- is rebuilt on top of it below, so nothing goes stale.
  const collapsed = createPillarCollapser()
  ...
  function buildPoints3DFC(records, nowMs) {
    const feats = []
    for (const r of collapsed(records)) {
      const tier = rssiTier(r.rssi, currentOffset())
